...
export default function ProduksiPage() {
  ...
  const { produkList, getResepByProdukId, addProduk, updateProduk, produksi, deleteProduk, loading } = useProduk();
  ...
  const [editResepItems, setEditResepItems] = useState<ResepItem[]>([
    { bahan_id: 0, jumlah_bahan: 0 }
  ]);
  ...
  const checkStokCukup = (produkId: number, jumlahProduksi: number = 1) => {
    const resep = getResepByProdukId(produkId);
    return resep.every(r => {
      const bahan = getBahanById(r.bahan_id);
      return bahan && bahan.stok_total >= r.jumlah_bahan * jumlahProduksi;
    });
  };
  ...
  const openEditDialog = () => {
    ...
    setEditResepItems(currentResep.map(r => ({
      bahan_id: r.bahan_id,
      jumlah_bahan: Math.round(r.jumlah_bahan)
    })));
  ...
  const handleEditImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setEditProduk({ ...editProduk, gambar: file });
      const reader = new FileReader();
      reader.onloadend = () => {
  ...
  const handleSubmitEditProduk = async () => {
    if (!selectedProduk) return;
    ...
    <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {getResepByProdukId(selectedProduk.produk_id).map(resep => {
                      ...
                      const jumlahDibutuhkan = resep.jumlah_bahan * jumlahProduksi;
                      const cukup = bahan && bahan.stok_total >= jumlahDibutuhkan;
                        ...
                        >
                          ...
                          <div className="text-right">
                            <p className="font-medium">
                              {Math.round(jumlahDibutuhkan)} / <span className="text-muted-foreground">{Math.round(bahan?.stok_total || 0)}</span>
                            </p>
          ...
          <div className="space-y-6 py-4 overflow-y-auto flex-1 px-1">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onInput={(e) => { const target = e.target as HTMLInputElement; target.value = target.value.replace(/[.,]/g, ''); }}
                className="rounded-xl"
              />
            </div>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="sm:max-w-2xl rounded-2xl max-h-[90vh] flex flex-col">
          <DialogHeader>
            <DialogTitle className="font-space text-xl flex items-center gap-3">
              <div className="p-2 rounded-xl bg-gradient-to-br from-y2k-teal-light to-y2k-mint">
          ...
          <div className="space-y-6 py-4 overflow-y-auto flex-1 px-1">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setEditProduk({ ...editProduk, nama_produk: e.target.value })}
                className="rounded-xl"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                min="0"
                placeholder="Masukkan harga jual"
                value={editProduk.harga_jual}
                onChange={(e) => setEditProduk({ ...editProduk, harga_jual: e.target.value })}
                onKeyDown={(e) => { if (e.key === '.' || e.key === ',') e.preventDefault(); }}
                onInput={(e) => { const target = e.target as HTMLInputElement; target.value = target.value.replace(/[.,]/g, ''); }}
                className="rounded-xl"
              />
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  onClick={() => setEditResepItems([...editResepItems, { bahan_id: 0, jumlah_bahan: 0 }])}
                  className="rounded-xl"
                >
                  <Plus className="w-4 h-4 mr-1" />
                  Tambah Bahan
                  ...
                  <div key={index} className="flex gap-3 items-end p-3 rounded-xl bg-muted/30 border border-border">
                    <div className="flex-1 space-y-2">
                      ...
                      <Select
                        value={item.bahan_id.toString()}
                        onValueChange={(value) => {
                          const updated = [...editResepItems];
                          updated[index].bahan_id = parseInt(value);
                          setEditResepItems(updated);
                        }}
                      >
                        <SelectTrigger className="rounded-xl">
                          <SelectValue placeholder="Pilih bahan" />
                    ...
                    <div className="w-28 space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          const updated = [...editResepItems];
                          updated[index].jumlah_bahan = parseInt(e.target.value) || 0;
                          setEditResepItems(updated);
                        }}
                        onKeyDown={(e) => { if (e.key === '.' || e.key === ',') e.preventDefault(); }}
                        onInput={(e) => { const target = e.target as HTMLInputElement; target.value = target.value.replace(/[.,]/g, ''); }}
                    ...
                    {editResepItems.length > 1 && (
                      <Button
                        type="button"
                        variant="ghost"
                        size="icon"
                        onClick={() => setEditResepItems(editResepItems.filter((_, i) => i !== index))}
                        className="rounded-xl text-destructive hover:text-destructive hover:bg-destructive/10"
                      >
                        <Trash2 className="w-4 h-4" />
                      </Button>
          ...
          <DialogFooter className="mt-auto">
            ...
            <Button
              onClick={handleSubmitEditProduk}
              className="bg-gradient-to-r from-y2k-pink to-y2k-purple hover:opacity-90 rounded-xl"
            >
              <Check className="w-4 h-4 mr-2" />
              Update Produk
